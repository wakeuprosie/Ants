...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # check if current place has bees
        moves = 0
        ...
        while temp_place.entrance != beehive:
            temp_place = temp_place.entrance
            moves += 1
            if temp_place.bees != []:
                if self.min_range == None and self.max_range == None:
                    return rANTdom_else_none(temp_place.bees)
                elif self.min_range != None and self.max_range != None:
                    if moves in range(self.min_range, self.max_range + 1):
                        return rANTdom_else_none(temp_place.bees)
                elif self.min_range != None:
                    if moves >= self.min_range:
                        return rANTdom_else_none(temp_place.bees)
                elif self.max_range != None:
                    if moves <= self.max_range:
                        return rANTdom_else_none(temp_place.bees)
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # memorize the place in case the ant dies and the place attribute becomes None
        temp_place = self.place

        # ant takes damage
        Ant.reduce_armor(self, amount)

        # if the place also has bees, they take damage
        if temp_place.bees != []:
            copy_bees = list(temp_place.bees) #copy the list of bees
            bee_iter = iter(list(temp_place.bees))
            if self.armor == 0: # if the ant just died
                # go through every bee and reduce its armor until no bees
                for bee in copy_bees:
                    Insect.reduce_armor(next(bee_iter), (amount+self.damage)) #call the reduce_armor function on it
                temp_place.bees = list(copy_bees)
            else:
                for bee in copy_bees:
                    Insect.reduce_armor(next(bee_iter), (amount))
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        "*** YOUR CODE HERE ***"
        bee.armor = 0
        self.digesting = int(self.time_to_digest)
        # END Problem 6

    def action(self, gamestate):
        # BEGIN Problem 6
        # check if ant is digesting
        "*** YOUR CODE HERE ***"
        if self.digesting > 0:
            self.digesting -= 1
        else:
...
class ScubaThrower(ThrowerAnt):
    ...
    def __init__(self, armor=1, food_cost=6):
        return ThrowerAnt.__init__()
# END Problem 9
